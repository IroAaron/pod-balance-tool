...
- **Билды** (`/builds`, `/builds/:id`) — список карточек: заголовок (иконка+название, клик → карточка билда), под ним — кликабельные спрайты предметов, и раздел «Возможно связано с» (кликабельные иконки других билдов). Поиск/фильтр по тегу/сортировка/галочка «Учитывать прокачки (+/++)» (по умолчанию выключена — оба алгоритма ниже игнорируют уровни прокачки, пока не включишь)/кнопки «Предложить билды» (автокластеризация по тегам/id) и «Собрать билды от очков» (каскад от PlayerScore, см. ниже)/«+ Создать билд». `/builds/:id` — форма редактирования (имя/иконка/описание/состав/**ручные связи с другими билдами**).
